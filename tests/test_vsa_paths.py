...
def test_paths_use_configured_root_and_canonical_stages(tmp_path, monkeypatch):
    ...
    expected_csv = tmp_path / "Demo" / "csv" / "LOT-1" / "STAGE3" / "CMP-1.csv"
    assert csv_path("Demo", "LOT-1", "STAGE3", "CMP-1") == expected_csv
    assert DYNAMIC_STAGES == ("STAGE1", "STAGE2", "STAGE3", "STAGE4", "STAGE5", "STAGE7")


def test_map_image_discovers_supported_extension(tmp_path, monkeypatch):
    monkeypatch.setenv(DATA_ROOT_ENV, str(tmp_path))
    expected = tmp_path / "Demo" / "map" / "LOT-1" / "STAGE1" / "CMP-1.jpg"
    expected.parent.mkdir(parents=True)
    expected.write_bytes(b"sample")

    assert map_image_path("Demo", "LOT-1", "STAGE1", "CMP-1") == expected
...
def test_roi_path_rejects_package_traversal(tmp_path, monkeypatch):
    monkeypatch.setenv(DATA_ROOT_ENV, str(tmp_path))
    with pytest.raises(ValueError):
        roi_image_path("Demo", "LOT-1", "STAGE1", "CMP-1", "../42")


def test_map_image_supports_dotted_component_ids(tmp_path, monkeypatch):
    monkeypatch.setenv(DATA_ROOT_ENV, str(tmp_path))
    expected = tmp_path / "Demo" / "map" / "LOT-1" / "STAGE1" / "CMP.1.png"
    expected.parent.mkdir(parents=True)
    expected.write_bytes(b"sample")
